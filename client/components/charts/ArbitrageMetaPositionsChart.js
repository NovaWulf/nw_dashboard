...
export default function ArbitrageMetaPositionsChart({version,basket}) {
  ...
  query ($version: Int!, $basket: String!){
    ...
    cointegrationModelWeights(version:$version,sequenceNumber:0,basket:$basket) {
      id
      assetName
    }
    backtestPositions(version: $version,basket: $basket) {
      ts
      v
      is
    }
  ...
  const { getMetaBreakpoints, backtestPositions,cointegrationModelWeights } = data || {};
