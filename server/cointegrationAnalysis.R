...
startTime = as.numeric(as.POSIXct("2022-06-20 "))
...
jo=ca.jo(dataMat,type= "trace",ecdet = "none",spec="transitory")
summary(jo)
plot1 = xyplot(close.x~starttime,bothDat,type="l", auto.key = TRUE, main = "double axis plot of OP vs ETH futures")
plot2 = xyplot(close.y~starttime,bothDat,type="l",auto.key = TRUE)
...
xyplot(spread + meanSpread~starttime,bothDat,type = "l",auto.key = T,main= "mean reverting portfolio")
...
bothDat$PnL = PnL
plot3 = xyplot(spread + meanSpread + upper+lower~starttime,bothDat,type = "l",auto.key = T,main= "mean reverting portfolio")
plot4 = xyplot(PnL~starttime,bothDat,type = "l",main= "PnL of arb strategy with 1000 ETH")
plot3
plot4
